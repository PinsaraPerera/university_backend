...
@router.post(
    "/create",
    response_model=lecture_schema.LectureBase,
...
def update(
    lecture_id: int,
    request: lecture_schema.LectureUpdate,
    db: Session = Depends(get_db),
    # current_user: admin_schema.Admin = Depends(oauth2.get_current_user),
):
...
@router.get(
    "/getTeacher/{teacher_id}", response_model=List[lecture_schema.LectureResponse]
)
...
@router.get(
    "/getDegree/{degree_id}", response_model=List[lecture_schema.LectureResponse]
)
...
@router.get(
    "/getYear/{academic_year}", response_model=List[lecture_schema.LectureResponse]
)
